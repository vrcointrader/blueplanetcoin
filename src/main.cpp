...
uint256 hashGenesisBlock("0x2647b227f93b11c01edb2f4932c445a64a4420e28800933191e90b8de797ff91");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // blueplanetcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "blueplanetcoin Signed Message:\n";
...
{
    // blueplanetcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight == 1){
        nSubsidy = nSubsidy * 52631500 * 5  / 100 / 50;
     }
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // blueplanetcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // blueplanetcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // blueplanetcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x2647b227f93b11c01edb2f4932c445a64a4420e28800933191e90b8de797ff91");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1405492167;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084744928;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("blueplanetcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("blueplanetcoinMiner : ProcessBlock, block not accepted");
    }
...
void static LitecoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running blueplanetcoin with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("blueplanetcoinMiner terminated\n");
        throw;
    }
}
